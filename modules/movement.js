...
export const updateMovement = (delta, controls, camera, walls, statue) => {
  ...
  // const statueBoundingBox = statue.BoundingBox;
  ...
  if (checkCollision(camera, walls, statue)) {
...
export const checkCollision = (camera, walls, statue) => {
  // 1) Tạo bounding box cho player (camera)
  const playerBB = new THREE.Box3();
  const camPos = new THREE.Vector3();
  camera.getWorldPosition(camPos);
  playerBB.setFromCenterAndSize(
    camPos,
    // bạn có thể điều chỉnh kích thước box nhỏ/hẹp hơn nếu cần
    new THREE.Vector3(1, 2, 1)
  );

  // 2) Va chạm với statue?
  if (statue) {
    const statueBB = new THREE.Box3().setFromObject(statue);
    if (playerBB.intersectsBox(statueBB)) {
      console.log("collision with statue");
      return true;
    }
  }

  // 3) Va chạm với walls như cũ
  for (let wall of walls.children) {
    if (playerBB.intersectsBox(wall.BoundingBox)) {
      return true;
    }
  }
...
// export const checkCollision = (camera, walls, statueBoundingBox, distancetoStatue) => {
...
//   );
//   // const statuedistanceThreshold = 6.1;
//   // // Check collision trực tiếp giữa playerBoundingBox và statueBoundingBox
//   // if (statueBoundingBox && playerBoundingBox.intersectsBox(statueBoundingBox)) {
//   //   console.log('collision with statue');
//   //   return true;
//   // }

//   // Kiểm tra thẳng bounding box của camera với bounding box của statue
//   if (playerBoundingBox.intersectsBox(statueBoundingBox)) {
//     console.log('collision with statue');
//     return true;
//   }
